...
// enter text on the serial monitor OR BLUETOOTH device
...
#define	PR1(s, x)	{ Serial1.print(s); Serial1.println(x); }
...
{  static uint8_t	putIndex = 0;  
  ...
  {    newMessage[putIndex] = (char)Serial.read();    Serial1.write(newMessage[putIndex]);
...
{  static uint8_t	putIndex = 0;
  while (Serial1.available())  {
    newMessage[putIndex] = (char)Serial1.read();    Serial.write(newMessage[putIndex]);
    if ( (newMessage[putIndex] == '\n')  ||(putIndex >= BUF_SIZE-3)) 
      { for (int tel = 0; tel < END_SPACING; tel++) {  newMessage[putIndex++] = ' '; }	
      newMessage[putIndex] = '\0';      putIndex = 0;   
      testcode();
